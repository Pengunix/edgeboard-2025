...
public:
  ...
  bool process(cv::Mat imageBinary, cv::Mat imageCorrect, Tracking &track,
               uint64_t frameTime, float speed, int DisLeaving,
               std::vector<int> RoadWidth, float UpScale, float block_scale,
               float distance_block, bool DangerFlag, int DangerMode) {
    ...
    if (step == Step::Start) {
      ...
      // 锥桶检测
      cv::Mat hsv;
      cv::cvtColor(imageCorrect, hsv, cv::COLOR_BGR2HSV);
      // 定义黄色的HSV范围
    ...
    if (step == Step::Inside) {
      ...
      // 锥桶检测
      cv::Mat hsv;
      cvtColor(imageCorrect, hsv, cv::COLOR_BGR2HSV);
      // 定义黄色的HSV范围
      cv::Scalar lowerYellow(15, 100, 60);
      cv::Scalar upperYellow(35, 255, 255);
...
private:
   ...
   * @param left
   */
  void curtailTracking(Tracking &track, bool left, std::vector<int> RoadWidth) {
    if (left) // 向左侧缩进
    {
