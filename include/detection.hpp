...
public:
  ...
  void transposeAndCopyToTensor(const cv::Mat &src, NDTensor &dst) {
    ...
    for (int i = 0; i < 3; ++i) {
      std::memcpy(start + i * offset, channels[i].data, offset * sizeof(float));
    }
  }
  ...
  void run(const std::unordered_map<std::string, NDTensor> &feeds) {
    ...
    int numel = std::accumulate(im_shape.shape.begin(), im_shape.shape.end(), 1,
                                std::multiplies<>());
    input_tensors.push_back(Ort::Value::CreateTensor<float>(
        memory_info, im_shape.value(), numel, im_shape.shape.data(),
        im_shape.shape.size()));
    numel = std::accumulate(scale_factor.shape.begin(),
