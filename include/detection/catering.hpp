...
public:
  ...
  bool process(Tracking &track, cv::Mat &image,
    ...
    if (cateringEnable) // 进入岔路
    {
      if (!stopEnable && turning) {
        ...
        for (size_t i = 0; i < lines.size(); i++) {
          ...
          // 如果找到符合条件的直线，绘制并输出
          // cv::Mat imgRes = cv::Mat::zeros(Size(COLSIMAGE, ROWSIMAGE),
          // CV_8UC3); // 创建全黑图像 cv::line(imgRes, start, end,
          // cv::Scalar(0, 255, 0), 2); // 用黄色绘制符合条件的直线 std::cout <<
          // "检测到符合条件的斜线: (" << pt1.x << "," << pt1.y << ") -> (" <<
          // pt2.x << "," << pt2.y << ")" << endl; 显示结果 imshow("Detected
          // Lines", imgRes); waitKey(0);
        }
      }

      counterSession++;
      if (counterSession >
          (truningTime + travelTime + stopTime)) // 结束餐饮区域
      {
        counterRec = 0;
        counterSession = 0;
    ...
    {
      for (size_t i = 0; i < predict.size(); i++) {
        if (predict[i].type == LABEL_BURGER && predict[i].score > 0.4 &&
