...
class Layby {
public:
  Layby(int moment = 110, int stopTime = 200)
      : moment(moment), stopTime(stopTime) {}
    ...
    if (laybyEnable) // 进入临时停车状态
    {
      ...
      for (const cv::Vec4i &line : lines) {
        ...
        for (cv::Vec4i &merged : mergedLines) {
          ...
          {
            // 合并并延长线段
            int minX = std::min(std::min(merged[0], merged[2]),
                                std::min(line[0], line[2]));
            int maxX = std::max(std::max(merged[0], merged[2]),
  ...
  void drawImage(Tracking track, cv::Mat &image) {
    ...
    if (laybyEnable)
      putText(image, "[1] Layby - ENABLE", cv::Point(10, 10),
